// components/PresaleStats.tsx - UPDATED
"use client";
  ...
  const fetchPresaleStats = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        console.log('⚠️ API not ready, using mock data');
      ...
      console.log('Using mock presale stats for development');
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex justify-between text-xs text-white/80 mt-1">
            ...
            <span>
              {isCapReached ? '🎉 Presale Complete!' : 
                `${formatNumber(presaleCap - stats.totalCPDSold)} CPD remaining`}
            </span>
          </div>
        </div>
        ...
        {stats.error === null && stats.totalRaised > 0 && (
          <div className="p-3 bg-green-500/20 border border-green-500/30 rounded-lg text-center text-sm">
            <span className="text-green-300">✅ Using mock data for development</span>
          </div>
        )}
      </div>
    </div>
