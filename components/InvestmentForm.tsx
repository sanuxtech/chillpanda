...
import { sendUSDT, getUSDTBalance, sendTestUSDT } from "@/lib/solana/usdtpayment";
...
const InvestmentForm = ({ address, onInvestment }: InvestmentFormProps) => {
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (isDevnet) {
      ...
      try {
        ...
        usdtTxHash = `devnet_test_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <h3 className="text-xl font-bold mb-4">Invest in Chillpanda Presale</h3>
