...
const usdtMintAddress = process.env.NEXT_PUBLIC_USDT_MINT || USDT_MINTS[network as keyof typeof USDT_MINTS];
...
export async function sendUSDT(
  ...
  try {
    ...
    if (isDevnet) {
      console.log('🧪 Devnet mode: Using test transaction (no real USDT needed)');
    ...
    try {
      ...
      console.log('✅ Project token account:', projectTokenAccount.address.toString());
      ...
      if (isDevnet && error.message.includes('TokenAccountNotFoundError')) {
        console.log('⚠️ No USDT token account on devnet, using test transaction');
        return await sendTestUSDT(connection, sender, amount, signTransaction);
      }
      throw error;
    }
    ...
    if (error.message.includes('TokenAccountNotFoundError')) {
      ...
      throw new Error(`Insufficient USDT balance. Please ensure you have enough USDT in your wallet.`);
